...
SAVE_PATH = "./py_module/test18_trainer/smishing_model.pt"
...
def _mask_account(m):
    ...
    # 1) 숫자 총 길이 최소 8 (너무 짧으면 계좌 아님)
    if len(digits) < 8:
        return s

    # 2) 그룹 중 최소 하나는 3자리 이상 (예: '01 23' 같은 패턴 방지)
    if max(len(g) for g in groups if g) < 3:
        return s

    # 3) 날짜/시간 문맥 근접: 직전이 '/' 이거나 직후가 ':' 이면 패스
    #    (예: '08/01 23:56'에서 '01 23'처럼)
    before = m.string[max(0, m.start()-1):m.start()]
    after  = m.string[m.end():min(len(m.string), m.end()+1)]
    if before == "/" or after == ":":
        return s
...
def clean_text2(text, use_spacing=True, spacing_func=spacing, max_len=500):
    ...
    text = RE_ACCOUNT.sub(_mask_account, text)   # <-- 여기만 함수 치환으로 교체
    ...
    mask_tokens = {"MASK", "URL", "PHONE", "OTP", "ACCOUNT", "MONEY", "WEB"}
    def _bracket_replacer(m):
        inner = m.group(1)
        return f"[{inner}]" if inner in mask_tokens else inner
    text = re.sub(r"\[(.*?)\]", _bracket_replacer, text)
...
def predict(text):
    ...
    sentences = RE_WEB_PREFIX.sub('[WEB] ', sentences)

    sentences = split_by_period_and_newline(sentences)
    sentences = [clean_text2(s, use_spacing=True, max_len=500) for s in sentences]
    ...
    for sent in sentences:
        encoded = tokenizer(sent, return_tensors='pt', truncation=True,
        ...
        with torch.no_grad():
            ...
            # ✅ 학습과 동일한 pooling 사용(CLS/SEP 제외)
            pooled = mean_pooling_excl_specials(output, attention_mask, input_ids, SPECIAL_IDS)
    ...
    if len(sentence_embeddings) == 0:
        ...
        final_embedding = torch.stack(sentence_embeddings, dim=0).mean(dim=0).unsqueeze(0)

    # ✅ 학습과 동일하게 L2 정규화
    from torch.nn.functional import normalize as l2_normalize
    final_embedding = l2_normalize(final_embedding, dim=-1)
    ...
    # safe_guard 및 threshold 로직은 그대로
